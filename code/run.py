...
import utils
import re
from racTrainer import RACTrainer 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-exp_weight', type = float, default = 0.17)
    parser.add_argument('-cl_weight', type = float, default= 0.07)
    ...
    if args.mode == 'prediction':
        racTrainer = RACTrainer(args)
        clsTrainer = CLSTrainer(args)
        num_classes = len(labels)
        print("Load models...")
        racTrainer.load(num_classes = num_classes, saved_model_path = args.saved_model_path+args.event_type+"/phase1.pt")
        ...
        cls_pred_p1, _, exp_pred_data, exp_pred_masked, exp_pred_probs = racTrainer.classify(prepro_data)
        ...
        if args.mode == 'train':
            ...
            racTrainer = RACTrainer(args, text_data, cls_data, exp_data)
            print(">>>>> Phase 1...........")
            train_exp_data, train_labels = racTrainer.train(saved_model_path = args.saved_model_path+args.event_type+"/phase1.pt")
            print(">>>>> Phase 2...........")
            ...
            for train_indices, remain_indices in kfold.split(text_data, cls_data):
                ...
                train_exp_pred_data, train_labels, valid_exp_pred_data, \
                    valid_labels, test_exp_pred_data, test_labels, [p1_cls_f1, p1_exp_f1] = racTrainer.eval(train_indices, valid_indices, test_indices)
                
                print(">>>>> Phase 2............")
                p2_cls_f1 = clsTrainer.eval(train_exp_pred_data, train_labels, valid_exp_pred_data, valid_labels, test_exp_pred_data, test_labels)
                print("--------------------------------------------------")
                fold+=1

                p1_cls_f1s.append(p1_cls_f1)
                p1_exp_f1s.append(p1_exp_f1)
                p2_cls_f1s.append(p2_cls_f1)
            
            print("================================================")
            print("Average cls p1: ", np.mean(p1_cls_f1s))
            print("Average exp p1: ", np.mean(p1_exp_f1s))
            print("Average cls p2: ", np.mean(p2_cls_f1s))
